...
from pyecharts import Line3D,Geo
...
def map(request):
    ...
    map1 = geomap()
    context = dict(
        myechart=map1.render_embed(),
        host=REMOTE_HOST,
        script_list=map1.get_js_dependencies()
    )
    return HttpResponse(template.render(context, request))
def geomap():
    ...
    geo = Geo("我家",  title_color="#fff", title_pos="center", width=1000,height=600, background_color='#404a59')
    attr,value = geo.cast(data)
    geo.add("", attr, value, visual_range=[0, 200], maptype='china',visual_text_color="#fff",symbol_size=20, is_visualmap=True)
    return geo

...
def  jiexi(link):                                       #网页爬取小说
    ...
    with open(r'C:\Users\Administrator\Desktop\计频分析\斗破苍穹.txt',"a",encoding='gb2312', errors='ignore') as f:   #出现打开错误，直接复制路径打开报错，是因为”\”符号在python中会被视为转义符，此时传入的路径就出现差错；那我们可以采取修改路径的方式，将”\”改为”/”...在含有转义符的字符串前加‘r’表示字符串内按原始含义解释，不做转义处理；这里的r相当于正则表达的操作
        f.write(str[1].text)
    f.close()
...
def count():                                            #统计字频
    with open (r'C:\Users\Administrator\Desktop\计频分析\斗破苍穹.txt','r') as f:
        str = f.read()
    words = jieba.cut(str)
...
def excel_write(count):                                           #创建excel文件  ，并且写excel  
    ...
    excel.save(r'C:\Users\Administrator\Desktop\计频分析\斗破字频统计表.xls') #保存excel，名字为2.xls
    print('写入完成.....') 
